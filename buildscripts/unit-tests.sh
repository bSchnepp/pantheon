...
mkdir build
cd build
scan-build -plist --force-analyze-debug-code -analyze-headers -o analyzer_reports cmake .. -DONLY_TESTS=ON -DCMAKE_C_FLAGS="-fprofile-arcs -ftest-coverage -O0 -g3 -pthread -latomic" -DCMAKE_CXX_FLAGS="-fprofile-arcs -ftest-coverage -O0 -g3 -pthread -latomic" -DCMAKE_CXX_LINK_FLAGS="${CMAKE_CXX_FLAGS} -latomic -lpthread" -DCMAKE_C_LINK_FLAGS="${CMAKE_C_FLAGS} -latomic -lpthread"
